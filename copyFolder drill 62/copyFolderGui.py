...
class Gui:
	def __init__(self, master):
		...
		try:
			...
			self.label_lastRanTimeStamp = tk.Label(self.master,text='N/A -- Never Ran')
			self.label_lastRanTimeStamp.grid(row=0,column=3,columnspan=3,padx=(27,0),pady=(10,0),sticky=E)
...
def transfer_last_24(source, destination):
	...
	for file in files:
		...
		if file.endswith('.txt'):
			...
			try:
				if timestamp < timeStampLastRan[0]:
					print('{:.>14.12}\t not copied.  Only includes files since last run.'.format(fileNameDisplay))
			except:
				if timestamp < unixUTC - 86400:
					print('{:.>14.12}\t not copied.  Only includes files in the last 24 hours'.format(fileNameDisplay))
